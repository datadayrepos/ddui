...
const hsv2hsl = function (hue: number, sat: number, val: number) {
  // Perform the assignment before the conditional expression.
  const hueAdjusted = (2 - sat) * val
  const saturation = (sat * val) / (hueAdjusted < 1 ? hueAdjusted : 2 - hueAdjusted) || 0

  return [
    hue,
    saturation,
    hueAdjusted / 2, // Use the adjusted hue value for the lightness calculation
  ]
}
...
export default class Color {
  ...
  constructor(options: Partial<ColorOptions> = {}) {
    for (const option in options) {
      // Type guard to ensure 'option' is a key of 'ColorOptions'
      if (hasOwn(options, option as keyof ColorOptions)) {
        // TypeScript understands that 'option' is a key of 'ColorOptions' and thus, a key of 'Color' as well.
        (this as any)[option] = options[option as keyof ColorOptions]
      }
    }
    if (options.value)
