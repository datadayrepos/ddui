...
import { flatMap, get, merge } from '@datadayrepos/lodashts'
...
import type { ElTooltipProps } from '/@/components/tooltip'
...
export function getRowIdentity<T>(row: T, rowKey: string | ((row: T) => any)): string {
  if (!row)
...
export const getKeysMap = function <T>(
  ...
  ;(array || []).forEach((row, index) => {
    arrayMap[getRowIdentity(row, rowKey)] = { index, row }
  })
  return arrayMap
}
...
export function isFixedColumn<T>(index: number, fixed: string | boolean, store: any, realColumns?: TableColumnCtx<T>[]) {
  let start = 0
  let after = index
...
export function getFixedColumnsClass<T>(namespace: string, index: number, fixed: string | boolean, store: any, realColumns?: TableColumnCtx<T>[], offset = 0) {
...
export function getFixedColumnOffset<T>(index: number, fixed: string | boolean, store: any, realColumns?: TableColumnCtx<T>[]) {
  const {
    direction,
    start = 0,
    after = 0,
...
export function ensurePosition(style, key: string) {
  if (!style)
    return
