...
function getItemFromCache(props: Props, index: number, listCache: ListCache): ListItem {
...
function findItem(props: Props, listCache: ListCache, offset: number) {
  const { items, lastVisitedIndex } = listCache
...
function bs(props: Props, listCache: ListCache, low: number, high: number, offset: number) {
  while (low <= high) {
...
function es(props: Props, listCache: ListCache, index: number, offset: number) {
  const { total } = props
  let exponent = 1
...
function getEstimatedTotalSize({ total }: Props, { items, estimatedItemSize, lastVisitedIndex }: ListCache) {
