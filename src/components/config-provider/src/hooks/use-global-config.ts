...
type ProvideFunction = <T>(key: InjectionKey<T> | string, value: T) => void

export function provideGlobalConfig(config: MaybeRef<ConfigProviderContext>, app?: App, global = false) {
  const inSetup = !!getCurrentInstance()
  const oldConfig = inSetup ? useGlobalConfig() : undefined

  let provideFn: ProvideFunction | undefined

  if (app && app.provide) {
    // Bind the app's provide method to the app instance
    provideFn = app.provide.bind(app) as ProvideFunction
  }
  else if (inSetup) {
    // Use Vue's provide function if in setup
    provideFn = provide as ProvideFunction
  }
  if (!provideFn) {
    debugWarn(
      'provideGlobalConfig',
