...
import { flatten } from '@datadayrepos/lodashts'
...
function isNormalDay(type = '') {
  return ['normal', 'today'].includes(type)
}

export function useBasicDateTable(props: BasicDateTableProps, emit: SetupContext<BasicDateTableEmits>['emit']) {
  ...
  interface CellCoordinate { columnIndex: number; rowIndex: number }
  ...
  const setCellMetadata = (
    ...
    const shouldIncrement = setDateText(cell, { columnIndex, count, rowIndex })
  ...
  const rows = computed(() => {
    ...
    buildPickerTable({ column: 7, row: 6 }, rows_, {
  ...
  const handleRangePick = (newDate: Dayjs) => {
    if (!props.rangeState.selecting || !props.minDate) {
      emit('pick', { maxDate: null, minDate: newDate })
      emit('select', true)
    }
    else {
      if (newDate >= props.minDate)
        emit('pick', { maxDate: newDate, minDate: props.minDate })
      else
        emit('pick', { maxDate: props.minDate, minDate: newDate })
...
export function useBasicDateTableDOM(props: BasicDateTableProps, {
  isCurrent,
  isWeekActive,
