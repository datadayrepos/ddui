...
function checkDateWithinRange(date: ConfigType) {
  return selectableRange.value.length > 0
    ? timeWithinRange(date, selectableRange.value, props.format || 'HH:mm:ss')
    : true
}
function formatEmit(emitDayjs: Dayjs) {
  if (
    defaultTime
    && !visibleTime.value
    && !isChangeToNow.value
...
function emit(value: Dayjs | Dayjs[], ...args: any[]) {
  if (!value) {
...
function handleDatePick(value: DateTableEmits, keepOpen?: boolean) {
...
function moveByMonth(forward: boolean) {
  const action = forward ? 'add' : 'subtract'
  innerDate.value = innerDate.value[action](1, 'month')
  handlePanelChange('month')
}

function moveByYear(forward: boolean) {
...
async function handleMonthPick(month: number) {
...
async function handleYearPick(year: number) {
...
async function showPicker(view: 'month' | 'year') {
...
function onTimePickerInputFocus() {
  timePickerVisible.value = true
}
function handleTimePickClose() {
  timePickerVisible.value = false
}

function getUnits(date: Dayjs) {
  return {
    hour: date.hour(),
    minute: date.minute(),
    second: date.second(),
...
function handleTimePick(value: Dayjs, visible: boolean, first: boolean) {
...
function handleVisibleTimeChange(value: string) {
...
function isValidValue(date: unknown) {
  return (
    dayjs.isDayjs(date)
    && date.isValid()
...
async function handleFocusPicker() {
...
function handleKeydownTable(event: KeyboardEvent) {
  const { code } = event
  const validCode = [
    EVENT_CODE.up,
    EVENT_CODE.down,
...
function handleKeyControl(code: string) {
