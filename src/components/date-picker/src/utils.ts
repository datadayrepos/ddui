...
export function getDefaultValue(defaultValue: DefaultValue, { lang, unit, unlinkPanels }: GetDefaultValueParams) {
  let start: Dayjs
...
export function buildPickerTable(dimension: Dimension, rows: DateCell[][], {
  columnIndexOffset,
  startDate,
  nextEndDate,
  now,
  ...
  for (let rowIndex = 0; rowIndex < dimension.row; rowIndex++) {
    ...
    for (let columnIndex = 0; columnIndex < dimension.column; columnIndex++) {
      ...
      setCellMetadata?.(cell, { columnIndex, rowIndex })
