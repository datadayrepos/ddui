...
import { get, isEqual, isNil, debounce as lodashDebounce } from '@datadayrepos/lodashts'
...
function useSelect(props: ISelectProps, emit) {
  // inject
  const { t } = useLocale()
  ...
  const filteredOptionsValueMap = computed(() => {
    ...
    filteredOptions.value.forEach((option, index) => {
      valueMap.set(getValueKey(getValue(option)), { index, option })
    })
    return valueMap
  })
