...
import type { ItemProps } from '/@/components/virtual-list'
  ...
  setup(props, { slots, expose }) {
    ...
    const isEqual = (selected: unknown, target: unknown) => {
      if (!isObject(target)) {
        ...
        // As we have already checked that target is an object, we now need to check if selected is also an object.
        if (isObject(selected)) {
          const { valueKey } = select.props
          return get(selected, valueKey) === get(target, valueKey)
        }
        else {
          // Here you can decide what should happen if selected is not an object.
          // For instance, return false since they can't be equal if one is an object and the other isn't.
          return false
        }
      }
    }
